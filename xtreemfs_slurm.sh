...
function startServer() {
  DIR_HOSTNAME=`scontrol show hostnames | head -n 1`
  srun -N1-1 --nodelist=$DIR_HOSTNAME xtreemfs_slurm_rstart.sh "env.sh" "DIR" "DIR"
  ...
  srun -N1-1 --nodelist=$MRC_HOSTNAME xtreemfs_slurm_rstart.sh "env.sh" "MRC" "MRC"
  
  counter=1
  for osd_hostname in `scontrol show hostnames | head -n $NUMBER_OF_NODES | tail -n +$(( $SKIP_NODE_COUNT + 1 ))`; do
    OSDNAME="OSD${counter}"
    srun -N1-1 --nodelist=$osd_hostname xtreemfs_slurm_rstart.sh "env.sh" "OSD" "$OSDNAME"
    counter=$(($counter+1))
  done   
...
function stopServerAndSaveLogs() {
  ...
  for osd_hostname in `scontrol show hostnames | head -n $NUMBER_OF_NODES | tail -n +$(( $SKIP_NODE_COUNT + 1 ))`; do
    OSDNAME="OSD${counter}"
    srun -N1-1 --nodelist=$osd_hostname xtreemfs_slurm_rstop.sh "env.sh" "$OSDNAME" "$SAVE_LOGS"
    counter=$(($counter+1))
  done
  
  MRC_HOSTNAME=`scontrol show hostnames | head -n ${SKIP_NODE_COUNT} | tail -n 1`
  srun -N1-1 --nodelist=$MRC_HOSTNAME xtreemfs_slurm_rstop.sh "env.sh" "MRC" "$SAVE_LOGS"
  
  DIR_HOSTNAME=`scontrol show hostnames | head -n 1`
  srun -N1-1 --nodelist=$DIR_HOSTNAME xtreemfs_slurm_rstop.sh "env.sh" "DIR" "$SAVE_LOGS"
